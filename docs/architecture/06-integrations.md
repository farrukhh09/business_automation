...
- `client.py` — `InstagramClient(settings, http: httpx.Client)`:
  ...
  - `send_image(recipient_id, public_url)` → `{"message": {"attachments": [{"type": "image", "payload": {"url": ...}}]}}` (jpeg/png, ≤8MB; в документации Meta форма этого поля непоследовательна — список покрывает и одну картинку, и десять, `docs/research/instagram.md`). Так уходит фото прайс-листа (03 §1.4): отдельное исходящее сообщение `message_type=IMAGE` с `media_url`, картинку Instagram скачивает по `{PUBLIC_BASE_URL}/api/media/{pl-…}`. Если картинка не ушла, текстовый ответ всё равно отправляется — это два разных сообщения.
