...
Часы выдачи (03 §1.3, 18.09.2026): `order_hours_start` / `order_hours_end` — в какие часы бот принимает время доставки/самовывоза; `null` — без ограничения. В `PUT` явный `null` у этих двух полей снимает ограничение (у остальных полей `null` = «не менять»). Конец раньше начала или равный ему → 422 `validation_error`. Неверная сохранённая пара при чтении отбрасывается целиком, остальные настройки не трогаются.

Выходные и упаковка (03 §1.3, 21.09.2026): `closed_weekdays` — выходные дни пекарни, номера `date.weekday()` (0 — понедельник … 6 — воскресенье); пустой список = работаем всегда, список сохраняется отсортированным и без повторов, все семь дней → 422 `validation_error`. `min_order_quantity` и `order_quantity_step` — минимум и кратность общего количества штук в заказе (целые ≥ 1, `1` = правило выключено). Сохранённое значение, ломающее проверку уровня модели (выходные или пара часов), отбрасывается **только своей группой полей** — остальные настройки остаются.
